...
<template>
    <form class="h-full divide-y divide-gray-200 flex flex-col bg-white shadow-xl">
            ...
            <div class="py-6 px-4 bg-indigo-700 sm:px-6">
                <div class="flex items-center justify-between">
                    ...
                    <div class="ml-3 h-7 flex items-center">
                        <button type="button" class="bg-indigo-700 rounded-md text-indigo-200 hover:text-white focus:outline-none focus:ring-2 focus:ring-white" @click="onClickXButton">
                ...
                <div class="px-4 divide-y divide-gray-200 sm:px-6">
                    <div class="space-y-6 pt-6 pb-5">
                        <div>
                            ...
                            <div class="mt-1">
                                <input type="text" v-model="projectName" name="project-name" id="project-name" class="block w-full shadow-sm sm:text-sm focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md" />
                            </div>
                        </div>
                        <div>
                            ...
                            <div class="mt-2 space-y-5">
                                <div class="relative flex items-start">
                                    <div class="absolute flex items-center h-5">
                                        <input id="privacy-public" v-model="privacy" name="privacy" aria-describedby="privacy-public-description" type="radio" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300" checked />
                                    </div>
        ...
        <div class="flex-shrink-0 px-4 py-4 flex justify-end">
            <button type="button" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" @click="onClickCancel">Cancel</button>
            <button type="button" class="ml-4 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" @click="onClickSave">Save</button>
        </div>
    </form>
</template>
...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const onClickXButton = () => {
            context.emit('clickXButton')
        }

        const onClickSave = () =>{
            context.emit('clickSave')
        }
